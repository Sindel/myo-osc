...
// usage: myo-osc osc.ip.address osc.port MAC0 MAC1
//
// http://samy.pl - code@samy.pl - 03/03/2014
// edited by Michele Abolaffio -- micheleabolaffio.wordpress.com -- michele.abolaffio@gmail.com - 2016-11-25
...
std::string MAC0 = "00-00-00-00-00-00"; //initialize MAC0
std::string MAC1 = "00-00-00-00-00-00"; //initialize MAC1
int arguments = 0;
...
//        knownMyos.push_back(myo);
 //       myo->setStreamEmg(myo::Myo::streamEmgEnabled);
    ...
    {
      if (arguments<3) // no mac addresses provided as arguments, use hardcoded ones.
      {
        ...
        else return 3; //unknown myo
      } //end of hardcoded mac addresses part
        if (arguments == 4) //if only one myo mac address is provided
        { if (mac == MAC0)
          {
            return 0; //first myo
          }
          else return 3; //undeclared others are labeled as "3"
        } //end of one myo declaration

        if (arguments == 5) //if two mac addresses are provided
        { if (mac == MAC0)
          {
            return 0; //first myo
          }
...
int main(int argc, char** argv)
{
  arguments = argc;
    ...
    try
    {
        if (argc != 5 && argc != 4 && argc != 3 && argc != 2 && argc != 1)
        {
            std::cout << "\nusage: " << argv[0] << " [IP address] <port> MAC0 MAC1\n\n" <<
            ...
            "edits by Michele Abolaffio -- http://micheleabolaffio.wordpress.com -- michele.abolaffio@gmail.com\n" << std::endl;
        ...
        else if (argc == 2) //specified IP address
        {
            int port = 7777;
            std::cout << "Sending Myo OSC to " << argv[1] << " on port 7777\n";
            transmitSocket = new UdpTransmitSocket(IpEndpointName(argv[1], port));
        }
        else if (argc == 3)
        {
            std::cout << "Sending Myo OSC to " << argv[1] << ":" << argv[2] << "\n";
            std::cout << "Last edit by Michele Abolaffio, 2017-11-25. \nmyo-osc directly bound to mac addresses of specific myos. \nMyo 0 is Left, Myo 1 is Right.\n\n" << std::endl;
            transmitSocket = new UdpTransmitSocket(IpEndpointName(argv[1], atoi(argv[2])));
        }
        else if (argc == 4)
        {
          MAC0 = argv[3]; //first mac address
            std::cout << "Sending Myo OSC to " << argv[1] << ":" << argv[2] << "\n";
            std::cout << "Last edit by Michele Abolaffio, 2017-11-25. \nmyo-osc directly bound to mac addresses of specific myos. \nMyo 0 is Left, Myo 1 is Right.\n\n" << std::endl;
            transmitSocket = new UdpTransmitSocket(IpEndpointName(argv[1], atoi(argv[2])));
        }
        ...
        {
          MAC0 = argv[3]; //first mac address
          MAC1 = argv[4]; //second mac address
            std::cout << "Sending Myo OSC to " << argv[1] << ":" << argv[2] << "\n";
            std::cout << "Last edit by Michele Abolaffio, 2017-11-25. \nmyo-osc directly bound to mac addresses of specific myos. \nMyo 0 is Left, Myo 1 is Right.\n\n" << std::endl;
            transmitSocket = new UdpTransmitSocket(IpEndpointName(argv[1], atoi(argv[2])));
        }
